...
# 데몬으로 돌 때도 봇과 같은 .env를 읽어 인증 토큰을 얻는다.
try:
    from dotenv import load_dotenv
    load_dotenv(ROOT / ".env")
except Exception:  # noqa: BLE001
    pass
# 로컬 봇에 WEB_AUTH_TOKEN이 켜져 있으면 같은 토큰으로 Basic 인증한다.
_AUTH_TOKEN = os.getenv("WEB_AUTH_TOKEN", "")
...
def _get(path: str, timeout: int = 15) -> dict:
    req = urllib.request.Request(BOT + path)
    if _AUTH_TOKEN:
        cred = base64.b64encode(f"live_sync:{_AUTH_TOKEN}".encode()).decode()
