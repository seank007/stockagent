...
(function () {
  ...
  // ---- 카테고리 구성 ----
  var CATS = [
    { id: "display", label: "테마·표시", items: [
      { pref: "theme", name: "테마", desc: "전체 색 테마", ctrl: "seg", options: [["dark", "다크"], ["light", "라이트"], ["oled", "OLED"], ["auto", "시스템"]] },
      { pref: "zoom", name: "화면 배율", desc: "글자·표 크기 (70~150%)", ctrl: "range", min: 0.7, max: 1.5, step: 0.05, fmt: function (v) { return Math.round(v * 100) + "%"; } },
      { pref: "font", name: "글꼴", desc: "모노스페이스 / 고딕(가독성)", ctrl: "seg", options: [["mono", "모노"], ["sans", "고딕"]] },
      { pref: "density", name: "정보 밀도", desc: "여백 크기", ctrl: "seg", options: [["normal", "보통"], ["compact", "컴팩트"]] },
    ...
    { id: "color", label: "색상·차트", items: [
      ...
      { pref: "chartHeight", name: "차트 높이", desc: "주식 차트 크기", ctrl: "seg", options: [["normal", "보통"], ["tall", "크게"]] }
    ]},
    { id: "ticker", label: "시세·데이터", items: [
      ...
      { pref: "refresh", name: "자동 새로고침", desc: "주식 시세 갱신 주기", ctrl: "seg", options: [["10", "10초"], ["30", "30초"], ["60", "1분"], ["off", "수동"]] }
    ]},
...
(function () {
  ...
  function krxState(now) {
    ...
    if (isWk && mins >= 540 && mins < 930) return { s: "장중", open: true };
    return { s: (isWk && mins < 540) ? "장전" : "장마감", open: false };
  }
  function tickClock() {
    ...
    if (!st.open) {
      ...
      extra = " · 개장 " + (hh ? hh + "시간 " : "") + pad(mm) + ":" + pad(ss);
    }
  ...
  function renderBell() {
    ...
    if (list) {
      ...
      list.innerHTML = h.length ? h.map(function (n) {
        return '<div class="hd-noti-item ' + esc(n.kind || "") + '"><div class="hd-noti-title">' + esc(n.title || "알림") +
          '</div><div class="hd-noti-body">' + esc(n.body || "") + '</div><div class="hd-noti-time">' + relTime(n.t) + "</div></div>";
      }).join("") : '<div class="hd-noti-empty">알림이 없습니다.</div>';
    }
  }
  ...
  function calcModal() {
    ...
    m.innerHTML =
      ...
      '<div class="modal-header">손익 계산기<button class="modal-close" id="calc-close">×</button></div>' +
      '<div class="calc-body">' +
      '<div class="calc-row"><label>매수가</label><input type="number" id="calc-buy" placeholder="원"></div>' +
      '<div class="calc-row"><label>매도가</label><input type="number" id="calc-sell" placeholder="원"></div>' +
      '<div class="calc-row"><label>수량</label><input type="number" id="calc-qty" placeholder="주"></div>' +
      '<div class="calc-out" id="calc-out"></div>' +
      '<div class="calc-note">수수료 0.015%(양방향) · 거래세 0.18%(매도) 반영 · 국내주식 기준</div>' +
      "</div></div>";
    ...
    function compute() {
      ...
      if (buy <= 0 || qty <= 0) { out.innerHTML = '<div class="calc-hint">매수가와 수량을 입력하세요.</div>'; return; }
      ...
      out.innerHTML =
        '<div class="calc-line"><span>매수금액</span><b>' + fmt(buyAmt) + " 원</b></div>" +
        '<div class="calc-line"><span>매도금액</span><b>' + fmt(sellAmt) + " 원</b></div>" +
        '<div class="calc-line"><span>수수료+세금</span><b>' + fmt(fee + tax) + " 원</b></div>" +
        '<div class="calc-line big ' + cls + '"><span>순손익</span><b>' + (pnl >= 0 ? "+" : "") + fmt(pnl) + " 원 (" + (pct >= 0 ? "+" : "") + pct.toFixed(2) + "%)</b></div>";
    }
  ...
  function build() {
    ...
    wrap.id = "hd-tools";
    wrap.innerHTML =
      '<span class="hd-clock closed" id="hd-clock" title="한국거래소 장 상태">KRX —</span>' +
      '<button class="hd-icon-btn" id="hd-calc" title="손익 계산기">🧮</button>' +
      '<span class="hd-bell-wrap"><button class="hd-icon-btn" id="hd-bell" title="알림">🔔<span class="hd-badge" id="hd-bell-badge" style="display:none">0</span></button>' +
      '<div class="hd-noti-panel" id="hd-noti-panel" hidden><div class="hd-noti-head">알림<button class="hd-noti-clear" id="hd-noti-clear">모두 지우기</button></div><div id="hd-noti-list"></div></div></span>';
    // 설정 버튼 앞에 삽입(설정 버튼이 아직 없으면 맨 끝)
